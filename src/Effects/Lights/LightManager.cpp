...
void LightManager::update(sf::View* ptr_view, sf::Time & p_dt){
	for (int i = 0; i < m_lights.size(); i++)
	{
		...
		{
			if (true)//(sf::Rect<float>(m_lights[i].position.x, m_lights[i].position.y, 1, 1).intersects(sf::Rect<float>(ptr_view->getCenter().x - ptr_view->getSize().x/2, ptr_view->getCenter().y - ptr_view->getSize().y/2, ptr_view->getSize().x, ptr_view->getSize().x)))
			{
				m_lights[i].m_draw = true;
			}else
			{
