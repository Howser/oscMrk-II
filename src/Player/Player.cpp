...
void Player::spell_attack(const Items & p_item, const sf::RenderWindow & p_window){
	...
	case Items::TestAOE:
		if (m_attackTimer.getElapsedTime().asSeconds() > GetSpeed(Items::TestAOE))
		{
			...
			AOE aoe = AOE(Items::TestAOE, _AOE::GetRadius(_SPELL::ToSpell(Items::TestAOE)), _AOE::GetDuration(_SPELL::ToSpell(Items::TestAOE)), sf::Vector2<float>(getPosition().x + (sf::Mouse::getPosition(p_window).x - 640), getPosition().y + (sf::Mouse::getPosition(p_window).y - 360)), ptr_tree);
